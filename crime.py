...
<body>
    <div class="container">
        <div class="header">
            <h1>🎯 Optimized Crime Detection System</h1>
        ...
        <div class="controls">
            <button class="btn" onclick="startDetection()" id="start-btn">🎥 Start Detection</button>
            <button class="btn btn-danger" onclick="stopDetection()" id="stop-btn" disabled>⏹️ Stop Detection</button>
        </div>
        ...
        <div class="detection-log">
            <h3>📝 Detection Log (Context-Aware)</h3>
            <div id="log-entries">
                ...
                .then(data => {
                    ...
                    if (data.detections && data.detections.length > 0) {
                        ...
                        if (latest.is_verified_crime) {
                            crimeAlert.innerHTML = '🚨 ' + latest.alert_level + ' ALERT: ' + 
                            ...
                            suspiciousAlert.innerHTML = '⚠️ SUSPICIOUS ACTIVITY: ' + 
                                latest.display_class + ' (' + (latest.final_confidence * 100).toFixed(1) + '%)';
...
class OptimizedVideoClassificationSystem:
    ...
    def load_model(self, model_path=MODEL_SAVE_PATH):
        ...
        try:
            ...
            if 'class_names' in checkpoint:
                ...
                print(f"✅ Found {len(self.class_names)} classes in model")
            else:
                print("❌ No class names found in checkpoint")
                return False
            ...
            print("✅ Optimized model loaded and ready!")
            return True
    ...
    def send_email_alert(self, subject, message):
        ...
        try:
            ...
            print("✅ Optimized crime alert sent!")
            return True
        except Exception as e:
        ...
        try:
            ...
            if (is_verified_crime and
                ...
                self.send_email_alert(
                    f"🚨 {alert_level} CRIME ALERT: {display_class.upper()}",
                    f"OPTIMIZED CRIME DETECTION ALERT!\n\n"
                    f"🕒 Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                    f"🎯 Activity: {display_class}\n"
                    f"📊 Final Confidence: {final_confidence:.1%}\n"
                    f"🔍 Context Score: {smoothed_context:.1%}\n"
                    f"🎯 Stability: {stability_score:.1%}\n"
                    f"⚠️ Alert Level: {alert_level}\n"
                    f"📝 Context Analysis: {context_reason}\n"
                )
...
def stop_detection():
    try:
        ...
        print("🛑 Optimized detection stopped")
...
def main():
    print("🎯 Optimized Crime Detection System Starting...")
    print(f"📱 Device: {classification_system.device}")
    ...
    if os.path.exists(MODEL_SAVE_PATH):
        ...
        if classification_system.load_model():
            ...
            print(f"📊 Model has {len(classification_system.class_names)} classes")

            if len(classification_system.class_names) > 0:
                print("📋 Class analysis with context awareness:")
    ...
    print("📧 Email alerts for context-verified crimes")
    print("🔍 Debug info: http://localhost:5000/debug_info")
    print("\n🧠 Key Optimizations:")
